...
H3_CONFIG="${H3_RECIPE_DIR}/ref2va_video_5s.json"
H3_INPUT="/workspace/sglang/outputs/minimax_h3_recipes/t2va_white_cat_5s.mp4"
H3_OUTPUT_DIR="/workspace/sglang/outputs/minimax_h3_recipes"
H3_OUTPUT="${H3_OUTPUT_DIR}/ref2va_video_5s.mp4"
H3_LOG_DIR="/workspace/sglang/logs/minimax_h3_recipes"
H3_RUN_STAMP="$(date +%Y%m%d-%H%M%S)"
H3_LOG="${H3_LOG_DIR}/ref2va-${H3_RUN_STAMP}.log"
...
if [[ ! -s "${H3_INPUT}" ]]; then
  echo "Missing Ref2VA reference video: ${H3_INPUT}" >&2
...
echo "Task: ref2va (video reference)"
