...
>
> 实现后修订（2026-07-15，实测驱动，详见 results 文档）：① §3.4 阶段 2 的「50 步 PSNR ≥35dB」
> 验收对精度变更型优化不适用（扩散采样混沌放大），修订为三重证据（算子对拍 + 2 步短程
> PSNR ≥35dB + 视觉验收）；② §3.2 的接口名以 plan/实现为准（`maybe_all_to_all_4d`/`comm_tag`
> 等）；③ ctx 按 repo 规则实现为 msgspec.Struct；④ 阶段 1 e2e 验证实际用 81 帧逐帧 MD5（强于
> 轨迹相似度）。
