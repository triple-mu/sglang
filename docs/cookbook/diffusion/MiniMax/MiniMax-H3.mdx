...
### Advanced: quality-gated video VAE fast path on Blackwell

On CUDA SM100 or newer (B200, B300, GB200/GB300, RTX PRO 6000) the video VAE
carries a fast path that is installed at load time and mounted per request.
A `"quality": "lossless"` request (the default) runs the reference operators
bit-for-bit. `"quality": "extra-high"` mounts, for that decode only:

- Decoder tile and temporal-window batching: adjacent equal-shaped decoder
  tiles and decode windows share one forward, in the original sample order and
  with time kept as its own axis.
- Fused Q/K RMSNorm+RoPE in the ViT decoder, and single-pass tile assembly,
  temporal blend write and output denormalization kernels (the last three are
  bit-exact against the reference operators).

`"quality": "high"` additionally mounts the encoder side for that encode:
encoder tile batching and fused GroupNorm+SiLU in the causal CNN encoder.
These change only the rounding order of the conditioning latents, but the
denoiser amplifies that: on 8x SM120 the worst frame of an fl2va request
landed at 28-38 dB PSNR against `lossless`, while the decode-only path stays
near 50 dB, so the encoder side follows the approximate tier.

Nothing is required on the command line beyond a resident SM100+ deployment;
send `"quality": "extra-high"` in the request (or `--quality extra-high` with
`sglang generate`). The server logs
`[H3 VAE] decode fast path mounted: quality=extra-high caps=enc8/dec64/win1 fp8=False`
when a request mounts the path (`fp8=True` once the FP8 decoder below is
installed) and `[H3 VAE] decode fast path: used=<n> fallback=<m>` when it
leaves the scope; a non-zero `fallback` means a kernel predicate refused a
layout and the reference operator ran instead. A `high` request logs the same
two lines with `encode` for the visual encoding stage.

The batch caps and a kill switch are environment variables:

| Variable | Default | Effect |
| --- | --- | --- |
| `SGLANG_DIFFUSION_DISABLE_MINIMAX_H3_VAE_FAST_PATH` | `false` | Do not install the fast path; every quality level runs the reference operators. |
| `SGLANG_DIFFUSION_MINIMAX_H3_VAE_ENCODER_TILE_BATCH` | `8` | Encoder tiles per forward, `1`-`64`. |
| `SGLANG_DIFFUSION_MINIMAX_H3_VAE_DECODER_TILE_BATCH` | `64` | Decoder tile x window x sample rows per forward, `1`-`64`. |
| `SGLANG_DIFFUSION_MINIMAX_H3_VAE_WINDOW_BATCH` | `1` | Adjacent equal-shaped decode windows per forward, `0`-`64`; `1` decodes one window at a time, `0` groups all of them. Pooling helped on NVLink B200 but was slower and used about 10 GB more on a PCIe SM120 host, so it stays opt-in. |

The caps bound the work per call, not the output size; lower them if batched
activations exceed memory. Out-of-range values are rejected at startup.
`--component-attention-backends video_vae=torch_cudnn_sdpa` is an optional,
independent choice for the decoder attention and applies at every quality
level.

Batching changes which cuBLAS and cuDNN algorithms run and the fused norms
change the rounding order, so `extra-high` output is not bit-identical to
`lossless` (decode-only: t2va worst-frame 49.7 dB PSNR, SSIM 0.993, audio
unchanged on 8x SM120, 4 steps); the fast path is never mounted for a
`lossless` request.
Benchmarks follow in a later revision.

#### Online FP8 decoder (deployment-level, lossy)

`--component-quantizations.video_vae fp8` converts the 144 linears of the 36
ViT decoder blocks to FP8 once at load: per-output-channel weight scales,
per-token activation scales and fp16 outputs through the `sgl_kernel` scaled
GEMM. The encoder, embedding, norms, attention and output projection keep
their precision. Startup rejects the flag unless the deployment runs on CUDA
SM100 or newer, keeps the default fp16 decode autocast (no
`--component-precisions.video_vae`, no `--disable-autocast`) and holds the
video VAE resident; pass `--component-residency video_vae=resident` when a
performance mode would offload it. A conversion failure aborts the load. The
FP8 GEMMs run at every quality level; `"extra-high"` and `"high"` requests
additionally run the decoder blocks on fused producers (residual add +
RMSNorm + FP8 quantization, SiLU-gated activation + FP8 quantization,
residual add + output LayerNorm) and the mount line reports `fp8=True`.

`SGLANG_DIFFUSION_MINIMAX_H3_VAE_OUTPUT_PROJECTION_FP16=1` rounds the input
of the decoder output projection to fp16 (fp32 accumulation). It is read once
at load and works with or without the FP8 decoder.

Both are deployment-level precision changes outside the quality contract:
`"lossless"` no longer reproduces the reference decoder bit-for-bit, and the
strict 4xH200 fingerprint behind `"quality": "high"` requires both to be off.
Measure against a `lossless` server on the target GPU before serving them.
...
- `"extra-high"`: includes the global fusion-only tier but does not enable
  Cache-DiT or another approximate optimization. On SM100+ GPUs MiniMax-H3
  mounts its quality-gated video VAE fast path at this level (see "Advanced:
  quality-gated video VAE fast path on Blackwell" under Deployment details);
  the denoise path has no request-gated fusion site and stays the same as
  `lossless`.
...
- On SM100+ GPUs, `quality: "extra-high"` mounts the quality-gated video VAE decode fast path (decoder tile and window batching, fused Q/K norm+RoPE and output kernels) and `"high"` adds the encoder side (encoder tile batching, fused GroupNorm+SiLU); `lossless` never mounts either. `SGLANG_DIFFUSION_DISABLE_MINIMAX_H3_VAE_FAST_PATH=1` skips the install, and the `SGLANG_DIFFUSION_MINIMAX_H3_VAE_*_BATCH` variables bound the batched work per call.
- `--component-quantizations.video_vae fp8` (online FP8 decoder, SM100+, resident fp16-autocast video VAE only) and `SGLANG_DIFFUSION_MINIMAX_H3_VAE_OUTPUT_PROJECTION_FP16=1` are deployment-level precision changes applied at load and outside the quality contract: they change the output at every quality level, including `lossless`, and the `quality: "high"` fingerprint requires both to be off. See "Advanced: quality-gated video VAE fast path on Blackwell".
