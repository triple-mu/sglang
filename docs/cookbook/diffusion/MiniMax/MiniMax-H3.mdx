...
`[H3 VAE] decode fast path mounted: quality=extra-high caps=enc8/dec64/win1`
when a request mounts the path and `[H3 VAE] decode fast path: used=<n>
fallback=<m>` when it leaves the scope; a non-zero `fallback` means a kernel
predicate refused a layout and the reference operator ran instead. A `high`
request logs the same two lines with `encode` for the visual encoding stage.
