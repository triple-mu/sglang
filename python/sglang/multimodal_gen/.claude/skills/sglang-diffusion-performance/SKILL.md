...
| **MiniMax-H3 online FP8 VAE decoder** | `--component-quantizations.video_vae fp8`; optional `SGLANG_DIFFUSION_MINIMAX_H3_VAE_OUTPUT_PROJECTION_FP16=1` | Converts the 144 ViT decoder block linears to FP8 once at load (per-channel weight scales, per-token activation scales, `sgl_kernel` scaled GEMM). At `--quality extra-high`/`high` the blocks also run on fused residual+RMSNorm+FP8 and SiLU-gated FP8 producers. The env var rounds the output-projection input to fp16 with fp32 accumulation. | Video decode stage only; measure on the target GPU | Lossy at every quality level, including `lossless`: a deployment decision outside the quality contract, and the `quality: "high"` fingerprint requires both off. Startup rejects it below CUDA SM100, without the default fp16 decode autocast, or with a non-resident video VAE (`--component-residency video_vae=resident`). Confirm with `fp8=True` in the `[H3 VAE] ... fast path mounted` log line. |
...
|---|---|---|---|
| MiniMax-H3 | 1344x768 resolved canvas, 5 seconds / 124 frames at 24 fps, 50 joint video/audio steps | H200: `--num-gpus 4 --ulysses-degree 4 --performance-mode speed --enable-torch-compile false --enable-breakable-cuda-graph false`; H100: TP2 + Ulysses2 | Root ID plus `--model-variant fl2va` for T2VA/FL2VA or `ref2va` for Ref2VA. Ulysses only; no Ring/CFG/SageAttention. Preserve tiled video-VAE decode. BCG is not part of the validated H3 recipe: warmup and serving can have different packed host boundaries, and a replay-capable experiment must still beat eager without excessive graph memory. Profile joint denoise, video VAE, audio VAE/vocoder, encoder, and collectives separately. On SM100+ `--quality extra-high` also mounts the quality-gated video-VAE fast path (tile/window batching, fused GroupNorm+SiLU, Q/K norm+RoPE, output kernels); confirm it with the `[H3 VAE] ... fast path mounted` and `used=/fallback=` log lines, keep `lossless` as the reference, and lower `SGLANG_DIFFUSION_MINIMAX_H3_VAE_DECODER_TILE_BATCH` if batched decode activations exceed memory. |
