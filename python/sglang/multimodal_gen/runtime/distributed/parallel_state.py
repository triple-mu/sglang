...
def destroy_model_parallel() -> None:
    ...
    from .device_communicators.flashinfer_ulysses_a2a import (
        shutdown_flashinfer_ulysses_a2a,
    )
    from .device_communicators.ipc_a2a import IPC_A2A
    from .parallel_groups import PROCESS_GROUP

    IPC_A2A.reset()
    # Same reason, and stricter: closing a PCIe Ulysses communicator is itself
    # collective over the Ulysses group, so it has to happen while that group
    # is still alive.
    shutdown_flashinfer_ulysses_a2a()
