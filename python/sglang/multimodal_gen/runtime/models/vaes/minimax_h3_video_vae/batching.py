...
def forward_many(values, forward: Callable, capacity: int, collector=None):
    ...
    outputs = []
    for start, stop in partitions(values, capacity):
...
def flat_decode_tiles(tiles, indices, forward: Callable, capacity: int):
    """Flatten local tile × window/sample work, then restore each tile's batch."""
    flat = []
    sizes = []
    ...
    decoded = forward_many(flat, forward, capacity)
    outputs = []
    start = 0
    for size in sizes:
...
def decode_windows(
    ...
    for start, stop in partitions(windows, limit * int(z.shape[0]), max_inputs=limit):
