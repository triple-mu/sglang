...
def test_adaln_hoist_is_off_unless_the_env_flag_is_set():
    """Both gates are required: the env flag and a verified GEMM shape.

    The tiny model's adaln GEMM is not one whose batched-M projection was
    measured bitwise identical, so the flag alone must not enable the hoist.
    """
