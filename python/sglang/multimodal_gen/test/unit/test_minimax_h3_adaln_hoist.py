...
def test_production_shape_adaln_gemm_batching_is_reported_not_assumed():
    """The real [M, 2688] x [2688, 96768] projection at M=1-3 vs batched M=97.

    Whether batching the M dimension keeps the GEMM bitwise identical is a
    property of the device, not just the shape: it holds on H200 and does not
    on sm_120 (2504 of 9386496 BF16 elements differ). So this records the
    verdict for the machine the suite runs on rather than asserting one;
    ``test_hoisted_denoise_loop_matches_per_step_loop_bitwise`` covers the part
    that must hold everywhere -- that a mismatch makes the plan fall back to
    the per-step chain rather than change the output.
    """
    ...
    mismatched = int((batched != expected).sum())
    print(
        f"prod adaln_proj M={sorted(set(step_rows))} vs M={sum(step_rows)} on "
        f"{torch.cuda.get_device_name()}: max_abs={max_abs:.3e} "
        f"max_rel={max_rel:.3e} mismatched={mismatched}/{batched.numel()} "
        f"-> plan build {'batches' if mismatched == 0 else 'goes per step'} here"
    )
...
def test_hoisted_denoise_loop_matches_per_step_loop_bitwise():
    ...
    This has to hold on every device, including those where the batched GEMM
    is not bit-exact -- there the first-sight gate must catch the mismatch and
    put the loop back on per-step projection.
    """
    if not torch.cuda.is_available():
        return
    torch.manual_seed(0)
    ...
    with (
        ...
        # cuBLAS may pick a different kernel for the same small GEMM after an
        # unrelated large one has run in the process, so establish that this
        # loop even has a bitwise baseline before demanding the hoist match it.
        repeat_video, repeat_audio = _run_tiny_denoise_loop(model, hoist=False)
        if not (
            torch.equal(repeat_video, base_video)
            and torch.equal(repeat_audio, base_audio)
        ):
            print("per-step loop is not reproducible in this process; skipping")
            return
        # The first hoisted run also pays the gate's reference projections, so
        # measure the second one, where the verdict is already cached.
        _run_tiny_denoise_loop(model, hoist=True)
        projected.clear()
        hoisted_video, hoisted_audio = _run_tiny_denoise_loop(model, hoist=True)
        hoisted_projections = list(projected)

    batched_rows = sum(base_projections) // num_projections
    print(
    ...
    # Steady state must be cheaper than per-step, whatever it costs exactly:
    # one batched projection per module when the gate accepted, none at all
    # once a plan memo is in play, and the per-step count when it rejected.
    if len(hoisted_projections) < len(base_projections):
        assert set(hoisted_projections) <= {batched_rows}
    else:
        assert len(hoisted_projections) == num_steps * num_projections
...
def test_adaln_hoist_is_off_unless_the_env_flag_is_set():
    ...
    with patch.dict(
        ...
        # A rejected batched GEMM only changes how the plan is built, not
        # whether there is one: the per-step chain is always available.
        model._adaln_hoist_gate.disable()
        assert model.can_hoist_step_adaln()
...
if __name__ == "__main__":
    ...
    test_production_shape_adaln_gemm_batching_is_reported_not_assumed()
    print("PASS test_production_shape_adaln_gemm_batching_is_reported_not_assumed")
