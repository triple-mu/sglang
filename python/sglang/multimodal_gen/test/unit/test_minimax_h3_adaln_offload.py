...
def _tiny_gemm_shapes(model: MiniMaxH3DiTModel) -> set:
    return {
        (
            model.blocks[0].adaln_proj.linear.input_size,
            model.blocks[0].adaln_proj.linear.output_size,
            model.final_layer.adaln_proj.linear.input_size,
            model.final_layer.adaln_proj.linear.output_size,
        )
    }
...
def _hoist_and_offload_enabled(model: MiniMaxH3DiTModel):
    """Both env flags on and the tiny model's GEMM shapes accepted."""
    return (
        patch.dict(
            os.environ,
            {
        ...
        patch(
            "sglang.multimodal_gen.runtime.models.dits.minimax_h3._ADALN_HOIST_VERIFIED_GEMM_SHAPES",
            _tiny_gemm_shapes(model),
        ),
    )
...
def test_plan_memo_skips_the_second_projection():
    ...
    env, shapes = _hoist_and_offload_enabled(model)
    with env, shapes, patch.object(MiniMaxH3AdalnProj, "project", counting_project):
